...
Finally, we shown in Table \ref{hdecayTHDMtype1} a comparison between our numerical evaluations and those made via $$\texttt{HDecay}$$ package \cite{Djouadi:2018xqq}, which the branching ratios of the Higgs boson decaying to pair of particles ($$b\bar{b}$$, $$s\bar{s}$$, $$c\bar{c}$$, $$t\bar{t}$$, $$\tau^+\tau^-$$, $$\mu^+\mu^-$$, $$gg$$, $$\gamma\gamma$$, $$Z\gamma$$, $$W^+W^-$$, $$ZZ$$) in the theoretical framework of the THDM-I are shown. Again, the Feynman rules needed for evaluations are shown in Table \ref{AcoplosTHDM}, where it can be seen that only two parameters are introduced. We take the same inputs for these free THDM-I parameters as in Ref. \cite{Djouadi:2018xqq}, namely,
...
In Table \ref{hdecayTHDMtype1}, the quantities in brackets are the results generated via $$\texttt{SpaceMath}$$. We observe that our results are identical to those $$\texttt{HDecay}$$, which is to be expected since we actually reproduced the relevant expressions of the decay widths of the Higgs boson reported in Ref. \cite{Djouadi:2005gi}.
